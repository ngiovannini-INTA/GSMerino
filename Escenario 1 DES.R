...
#founderGenomes = quickHaplo(nInd=3000, nChr=26, segSites=3000) #fast, only to test runs
    system.time(
...
genCor = matrix(c( 1,0.05,0,0.01,0.09,
                   0.05,1,0.15,0,0.65,
                   0,0.15,1,0.25,0.25,
                   0.01,0,0.25,1,0.1,
                   0.09,0.65,0.25,0.1,1), ncol=5)

SP$addTraitA(nQtlPerChr = 1000, mean=c(0.817,23,2,18,45), var=genVar, corA = genCor)
...
#Phenotype the founders, generate DES trait (threshold), supress phenotypes on dead individuals
founders = setPheno(pop=founders, h2=heritability)

umb <- mean(founders@pheno[,1]) - sd(founders@pheno[,1])*1.25 # threshold value 1.25*sd = 16% of area = 
...
#Males
sobM = 0.97 #Rams survival

repM = round(nMales*.35) #Rams replacement
...
males = selectInd(pop = founders, nInd = sum(nMales0,nMales1,nMales2,nMales3), use='pheno', sex='M', trait = 1) #filter alive ones (DES=1)
...
repH = round(nFemales*.21) #Ewe replacement proportion 
...
females = selectInd(pop = founders, nInd = sum(nDams0,nDams1,nDams2,nDams3,nDams4,nDams5), trait = 1, use='pheno', sex='F') #filter alive ones (DES=1)
...
econWt = c(927.4,29.3,531.1,-28.3,2.9) #(a) from "Actualización de objetivos... .pdf"
...
# Scenario 1 PED --------------------------------- 
#BLUP + reduced phenotypes (actual BLUP)
for (year in 11:21) {
  cat('Working on the year:',year,'\n')
  ...
  dams0 = attrition(dams0, p=0.35) ##aphenotyped females 65% of DES=1 
  dams0 = setPheno(dams0, h2 = c(0.20, 0.30, 0.45, 0.35), traits = c(2,3,4,5))
  ...
  dat <- subset(dat, AN >= 3) #Since what year are there records?
  #dat[is.na(dat)] <- -999
  
  #local
  #write.table(ped, file = "d:/PROVINO/temp/e1/ped.dat", sep=" ", row.names = F, col.names = F, quote=F, na='-999')
  #write.table(dat, file = "d:/PROVINO/temp/e1/dat.dat", sep=" ", row.names = F, col.names = F, quote=F, na='-999')
  ...
  #run BLUP in local windows
  #setwd('d:/PROVINO/temp/e1/')
  #shell("renumf90 renum.par")
  #shell("blupf90 renf90.par")
  
  #run BLUP in Linux server
  ...
  EBV.pob <- filter(VC.j, id %in% data4AllAnimals$id) %>%
  ...
  EBV <- filter(VC.j, id %in% sires0@id) %>%
    select(id,trait, solution)
  ...
  EBV <- filter(VC.j, id %in% dams0@id) %>%
    select(id,trait, solution)
  ...
  sires1  = selectInd(pop = sires0, nInd = nMales1, use = "ebv", sex = 'M', trait = selIndex, b=econWt) #BT to 2T
...
#collects information by repetition
#local windows
#write.table(out,paste('d:/PROVINO/temp/e1/','out_Rep',rep,'.csv', sep=''),sep=';',row.names = F)
#write.table(cor,paste('d:/PROVINO/temp/e1/','cor_Rep',rep,'.csv', sep=''),sep=';',row.names = F)
...
#collects al the information
#local windows
#write.table(out.i,"d:/PROVINO/temp/e1/out.csv", sep=';',row.names = F)
#write.table(cor.i,"d:/PROVINO/temp/e1/cor.csv", sep=';',row.names = F)
