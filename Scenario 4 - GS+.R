...
system.time(
  for (rep in 1:50) {
    ...
    #Males
    sobM = 0.97 #Ram survival
    
    repM = round(nMales*.35) #Ram replacement
    ...
    for (year in 1:10) {
      ...
      #se fenotipan los animales de 1 año de edad
...
#GS + PED+
    ...
    pre.snp.i <- cbind(id.ref, pre.snp.ref)
        ...
        for (year in 11:21) {
          ...
          #Local windows
          write.table(ped, file = "d:/PROVINO/temp/e4/ped.dat", sep=" ", row.names = F, col.names = F, quote=F, na='-999')
          write.table(dat, file = "d:/PROVINO/temp/e4/dat.dat", sep=" ", row.names = F, col.names = F, quote=F, na='-999')
          write.fwf(pre.snp.i, file = "d:/PROVINO/temp/e4/snp.dat", sep="", rownames = F, colnames = F, width = c(7,52000))
          
          #run BLUP in local windows
          setwd('d:/PROVINO/temp/e4/')
          ...
          sires3 = selectInd(pop = sires2, nInd = nMales3, use = 'rand') #4D que pasan a 6D
          sires2 = selectInd(pop = sires1, nInd = nMales2, use = 'rand') #2D que pasan a 4D
          sires1 = selectInd(pop = sires0, nInd = nMales1, use= "ebv", sex = 'M', trait = selIndex, b=econWt) #DL que pasan a 2D
          ...
          dams3 = selectInd(pop = dams2, nInd = nDams3, use = 'rand')
          dams2 = selectInd(pop = dams1, nInd = nDams2, use = 'rand')
          dams1 = selectInd(pop = dams0, nInd = nDams1, use = 'ebv', sex = 'F', trait=selIndex, b=econWt) #DL que pasan a 2D, se selecionan las mejores y pasan a ser madres de la sig. gen.
        ...
        file.rename('d:/PROVINO/temp/e4/renf90.inb',paste('d:/PROVINO/temp/e4/renf90',rep,'.inb',sep=''))  
       ...
       #temp files - windows
       write.table(out,paste('d:/PROVINO/temp/e4/','out_',rep,'.csv', sep=''),sep=';',row.names = F)
       write.table(cor,paste('d:/PROVINO/temp/e4/','cor_',rep,'.csv', sep=''),sep=';',row.names = F)
